...
func (c *Client) GetStories(ctx context.Context) ([]Item, error) {
	var items []Item
	err := pgxscan.Select(
		ctx,
		c.pool,
...
func (c *Client) GetJobs(ctx context.Context) ([]Item, error) {
	var items []Item
	err := pgxscan.Select(
		ctx,
		c.pool,
...
func (c *Client) Insert(ctx context.Context, item Item) error {
	sql := `
